...
git.log = function (params, callback) {
  ...
  // TODO: This is not proper way. Need to fix it
  // issue #25
  try {
    ...
    console.log(err);
    callback(null, {full_name: full_name, regex: regex, results: []});
    return;
  }
...
git.grep = function (params, callback) {
  ...
  try {
    ...
    var error_message = '> not found the pattern in ' + id + '\n';
...
git.clone = function (params, callback) {
  ...
  exec(command, function (error, stdout, stderr) {
    if (error) {
      process.chdir(root + full_name);
      exec('git pull', function (error, stdout, stderr) {
        process.chdir(cur_dir);
        if (error) {
          console.log("Error: " + full_name);
          callback(error);
        } else {
          console.log("Updated: " + full_name);
          callback(null);
        }
      });
    } else {
