...
git.clone = function (cloneUrl, fullName) {
  var command = 'git clone ' + cloneUrl + ' ' + fullName;
  var child;

  child = exec(command, function (error, stdoud, stderr) {
...
git.multipleClone = function (urlNameList) {
  urlNameList.forEach(function (element, index, array) {
    git.clone(element.clone_url, element.full_name);
  });
};
