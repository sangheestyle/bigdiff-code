 ...
 *
 * @param {object} url, collection, docs
 * @api public
 */
...
mongo.upsertDocs = function(params, callback) {
  MongoClient.connect(params.url, function(err, db) {
    assert.equal(null, err);
    var collection = db.collection(params.collection);
    async.timesSeries(params.docs.length, function(i, next) {
...
/**
 * Find documents for query and then return specific fields by fields.
 *
 * @param {object} url, collection, query, fields
 * @api public
 */
...
mongo.group = function(params, callback) {
  MongoClient.connect(params.url, function(err, db) {
    assert.equal(null, err);
    var collection = db.collection(params.collection);
    collection.aggregate(params.group, function(err, result) {
      db.close();
      assert.equal(null, err);
      callback(err, result);
    });
